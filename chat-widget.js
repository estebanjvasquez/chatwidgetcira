(function() {
    ...
    function initWidget() {
        ...
        const styles = `
            ...
            @keyframes cira-pulse { 0%,100% { opacity:0.5; } 50% { opacity:1; } }
            .cira-loader-inner { display:flex; align-items:center; gap:10px; }
            .cira-loader-inner .cira-spinner { width:15px; height:15px; min-width:15px; border:2.5px solid var(--chat-color); border-top-color:transparent; border-radius:50%; animation:cira-spin 0.7s linear infinite; }
            .cira-loader-inner span { font-size:13px; color:#888; font-style:italic; animation:cira-pulse 1.6s ease-in-out infinite; }
        `;
        ...
        function addMsg(text, type) {
            ...
            // Fix PDF data: URI links so they open correctly in all browsers
            if (type === 'bot') fixDataUriLinks(div);
        }

        // Intercept data:text/html links — browsers block direct navigation to them,
        // so we decode + write into a new window then trigger print for PDF.
            ...
            links.forEach(function(link) {
                ...
                link.addEventListener('click', function(e) {
                    ...
                    if (w) {
                        ...
                        let chars = '';
                        for (let i = 0; i < bytes.length; i++) chars += String.fromCharCode(bytes.charCodeAt(i));
        ...
        async function sendMsg() {
            ...
            loader.innerHTML = '<div class="cira-loader-inner"><div class="cira-spinner"></div><span>' + loadingText + '</span></div>';
